...
const re_mixin = /\[\]\(<([^]+?)>\)/g
const re_html_mixin = /(?: \x04)?<!--([^\x01]+?)\x01-->\s*?(<\w+(?:(?:\s+\w+(?:\s*=\s*(?:".*?"|'.*?'|[^'">\s]+))?)+\s*|\s*))(\/>|>)/g
const re_mixin_split = /\s+(?=\.[-_A-Za-z])|\s+(?=[#@$^])/g
const re_id_attr = /id=("|')[^"']+?\1/
const re_code_block = /^```([^]+?)[\n\r]+?```/gm
const re_code_inline = /`([^\n\r`]+?)`/g
const re_preserved_code = /<!--Code(Block|Inline)\x02 (\d+)-->/g
...
function preserveMixin(markdown) {
    ...
    let codeBlock = []
    let codeInline = []
    markdown = markdown.replace(re_code_block, (str, capture) => {
    ...
    markdown = markdown.replace(re_mixin, (str, mixin) => `<!-- ${mixin.replace(/[\n\r\s]+/g, '  ')} \x01-->` )
    ...
    // restore code blocks
    markdown = markdown.replace(re_preserved_code, (str, typeMark, number) => {
        if (typeMark === 'Block')
